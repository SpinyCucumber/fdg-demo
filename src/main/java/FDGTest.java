...
public class FDGTest {
	...
	// The window handle
	private long window;
	private Graph<ForceSimulation.Body> graph;
	...
	private void init() {
		...
		GLFWErrorCallback.create(new GLFWErrorCallbackI() {
			...
			public void invoke(int error, long description) {
				log.error(String.format("GLFW Error: %s Description: %s", error, MemoryUtil.memUTF8(description)));
			}
		}).set();
		...
		glfwSetKeyCallback(window, (window, key, scancode, action, mods) -> {
			...
			if ( key == GLFW.GLFW_KEY_R && action == GLFW_RELEASE )
				resetGraph();
		});
	...
	public void resetGraph() {
		graph.createRandomGraph(nNodes, nConns);
		Graph.randomizePositions(graph, windowWidth / 2 - 1, windowWidth / 2 + 1,
				windowHeight / 2 - 1, windowHeight / 2 + 1);
	}
	...
	private void loop() {
		...
		while ( !glfwWindowShouldClose(window) ) {
			...
			simulation.update(timeScale * (float) timer.getDeltaTime());
			renderer.draw();
